...
const opikClient = opikApiKey ? new Opik({
  apiKey: opikApiKey,
  projectName: "polar-universal",
  workspaceName: process.env.OPIK_WORKSPACE || "default",
}) : null;

if (opikClient) {
  console.log("Opik tracing initialized for project: polar-universal");
} else {
  console.log("OPIK_API_KEY not set - tracing disabled");
}
...
function ruleBasedAnalysis(request: ComplianceAnalysisRequest): {
  violations: ComplianceViolation[];
...
// Core Gemini API call - wrapped with Opik tracing when available
async function callGeminiAPICore(prompt: string): Promise<string | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.log("GEMINI_API_KEY not set - using rule-based analysis");
    return null;
  }
...
const callGeminiAPI = opikClient 
  ? track({ name: "gemini-compliance-analysis", type: "llm" }, callGeminiAPICore)
  : callGeminiAPICore;
...
function parseGeminiResponse(response: string): {
  ...
  try {
    ...
    console.log("Could not parse Gemini JSON response, using text analysis");
  }
...
Respond with JSON:
{
  ...
  if (shouldQuarantine) {
    console.log(`AI detected compliance breach for ${request.serial_id} - triggering quarantine`);
  ...
  let status: "VERIFIED" | "QUARANTINE" | "AUDIT_REQUIRED" | "PENDING";
  if (shouldQuarantine) {
    status = "QUARANTINE";
  ...
  return {
    ...
    analysis_model: geminiResponse ? "gemini-1.5-flash" : "rule-based-fallback",
...
// Export wrapped version with Opik tracing
export const analyzeCompliance = opikClient 
  ? track({ name: "pharma-compliance-analysis", type: "general" }, analyzeComplianceCore)
  : analyzeComplianceCore;
